...
from config import aircraft_root, aircraft_partco_root
...
class FGVCAircraft(Dataset):
    ...
    patch_folder = aircraft_partco_root
    ...
    def __getitem__(self, index):
        ...
        # 1st order part-level correspondence labels
        # get patch correspondence label
        patch_sample = os.path.basename(path).split('.jpg')[0] + '_label.png'
        patch_path = os.path.join(self.patch_folder, patch_sample)
        ...
        flip_prob = np.random.choice([0.0, 1.0]) if getattr(self, 'random_hflip', True) else 0.0
        if flip_prob == 1.0:
            sample = hflip(sample)
            patch_label = hflip(patch_label)

        # convert patch label from pil to tensor
        ...
        return sample, target, patch_label, self.uq_idxs[index]
