"""Ported verbatim from the partco repo (data/get_datasets.py).

Deltas: imports rewired to ``data.partco.*``; ``get_dataset_funcs`` scoped to
the fine-grained datasets whose part-label datasets are ported here.
``get_class_splits`` is kept verbatim.
"""
from data.partco.data_utils import MergedDataset

from data.partco.stanford_cars import get_scars_datasets
from data.partco.cub import get_cub_datasets
from data.partco.fgvc_aircraft import get_aircraft_datasets
from data.partco.pets import get_pets_datasets
...
def get_datasets(dataset_name, train_transform, test_transform, args):
    ...
    if dataset_name not in get_dataset_funcs.keys():
        raise ValueError(
            f"'{dataset_name}' has no ported part-label dataset in data/partco "
            f"(available: {sorted(get_dataset_funcs)}). Port it from the partco "
            f"repo following data/partco/__init__.py."
        )
