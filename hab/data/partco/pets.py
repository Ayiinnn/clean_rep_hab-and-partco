...
from data.partco.data_utils import subsample_instances
from config import pets_root, pets_partco_root
...
class OxfordIIITPet(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any]:
        ...
        # get patch correspondence label
        # # 1st order patch labels
        # patch_sample = path.split('.jpg')[0] + '_label_2.png'
        # patch_sample = patch_sample.split('/')
        # patch_sample = os.path.join(patch_sample[2], 'first_order_labels', patch_sample[-1])
        
        # 2nd order patch labels
        patch_sample = path.split('.jpg')[0] + '_label_2.png'
        patch_sample = patch_sample.split('/')
        patch_path = os.path.join(self.patch_folder, 'second_order_labels', patch_sample[-1])

        patch_label = self.loader(patch_path)
        patch_label = patch_label.convert('L')

        # flip img & patch_label
        # [hypartco] random_hflip switch: True (default) = original PartCo behavior.
        flip_prob = random.choice([0.0, 1.0]) if getattr(self, 'random_hflip', True) else 0.0
        if flip_prob == 1.0:
            sample = hflip(sample)
        ...
        return sample, target, patch_label, self.uq_idxs[idx]
...
def get_pets_datasets(train_transform, test_transform, train_classes=range(19), prop_train_labels=0.8,
    ...
    whole_training_set = OxfordIIITPet(root=pets_root, transform=train_transform, split='trainval', download=True)  # len = 3680
