...
from config import imagenet_root, imagenet_partco_root


class ImageNetBase(torchvision.datasets.ImageFolder):
    patch_folder = imagenet_partco_root

    def __init__(self, root, transform, class_map=None):
    ...
    def __getitem__(self, item):
        ...
        # get patch label
        patch_label_path = os.path.join(self.patch_folder, 'second_order_labels', f"img_{uq_idx}_class_{self.class_map[label]}_label_2.png")
        # load patch label from path to tensor
        patch_label = self.loader(patch_label_path)
        patch_label = patch_label.convert('L')
        # convert patch_label from pil to tensor
        patch_label = torch.tensor(np.array(patch_label))

        return img, label, patch_label, patch_label, uq_idx
...
def get_imagenet_100_datasets(train_transform, test_transform, train_classes=range(80),
    ...
    imagenet_training_set = ImageNetBase(root=os.path.join(imagenet_root, 'train'), transform=train_transform, class_map=subsampled_100_classes.tolist())
    ...
    test_dataset = ImageNetBase(root=os.path.join(imagenet_root, 'val'), transform=test_transform, class_map=subsampled_100_classes.tolist())
