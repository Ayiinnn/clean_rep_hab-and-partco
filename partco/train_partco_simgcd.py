...
from models.partco_loss import PatchUnConLoss, PatchSupConLoss
...
def train(student, patch_projector, train_loader, test_loader, unlabelled_train_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            # fix-qxz: images, class_labels, patch_label_1, patch_label_2, uq_idxs, mask_lab = batch
            ...
            # fix-qxz: patch_label_2 = patch_label_2.cuda(non_blocking=True)
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                patch_features = student[0].forward_features(images.chunk(2)[0])['x_norm_patchtokens'] # [B. 256, 768]
                ...
                if epoch < args.warmup_teacher_temp_epochs:
                    ...
                    patch_unsup_con_loss = PatchUnConLoss(
                        dynamic_threshold=True, 
                        # hard_negative_weight=1.0
                    )(patch_out, patch_label_1, class_logits, mask_lab, epoch=epoch)
                ...
                if epoch < args.warmup_teacher_temp_epochs:
                    ...
                    loss += 0.5 * ((1 - args.sup_weight) * patch_unsup_con_loss + args.sup_weight * patch_sup_con_loss)
            # Train acc
        ...
        # fix-qxz: if old_acc_test > best_test_acc_lab:
        if old_acc > best_test_acc_lab:
            
            args.logger.info(f'Best ACC on old Classes on unlabelled train set: {old_acc:.4f}...')
...
def test(model, test_loader, epoch, save_name, args):
    ...
    for batch_idx, batch in enumerate(tqdm(test_loader)):
        # fix-qxz: be compatible with different dataset return length
...
if __name__ == "__main__":
    ...
    # change-qxz: use local cached DINOv2 repo to avoid torch.hub network/check/repo-lock hangs.
    backbone = torch.hub.load('/root/.cache/torch/hub/facebookresearch_dinov2_main', 'dinov2_vitb14', source='local')
    ...
    # TRAIN
    # ----------------------
    train(model, patch_projector, train_loader, None, test_loader_unlabelled, args)
